...
module.exports = async ({
  ...
  let prodPackages = `${manager} debug @sinclair/typebox http-errors pino-pretty ${
    graphql
  ...
  switch (database) {
    ...
    case 'postgres':
      devPackages += ' prisma '
      prodPackages += ' @prisma/client pg pg-hstore'
      break
    case 'mysql':
      devPackages += ' prisma '
      prodPackages += ' @prisma/client mysql2'
      break
    case 'mariadb':
      devPackages += ' prisma '
      prodPackages += ' @prisma/client mariadb'
      break
    case 'sqlite':
      devPackages += ' prisma '
      prodPackages += ' @prisma/client sqlite3'
      break
    case 'sqlServer':
      devPackages += ' prisma '
      prodPackages += ' @prisma/client tedious'
      break
    default:
      throw new Error('Database not supported')
  }

  devPackages += `@types/debug @types/http-errors \
@types/node \
