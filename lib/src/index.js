...
module.exports = async ({
  ...
  switch (database) {
    case 'mongo':
      ...
      break
    case 'postgres':
      prodPackages += ' sequelize pg pg-hstore'
      break
    case 'mysql':
      prodPackages += ' sequelize mysql2'
      break
    case 'mariadb':
      prodPackages += ' sequelize mariadb'
      break
    case 'sqlite':
      prodPackages += ' sequelize sqlite3'
      break
    case 'sqlServer':
      prodPackages += ' sequelize tedious'
  }
...
webpack-node-externals`
  ...
  try {
    ...
    const functions = [
      ...
      nodemon(projectName),
      eslint({ projectName, tests, dbIsSQL: database !== 'mongo' }),
      webpack(projectName),
      docker(projectName),
