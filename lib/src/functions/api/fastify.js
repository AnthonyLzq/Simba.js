...
const types = async ({ projectName, graphQL, dbIsSQL }) => {
  ...
  if (!dbIsSQL)
    processes.push(
      writeFile(
        `${projectName}/src/@types/models/user.d.ts`,
        t('types/models/user.d.ts.ejs')
      )
    )
 ...
 * @param {Boolean} args.dbIsSQL
 */
const network = async ({ projectName, email, graphQL, dbIsSQL }) => {
...
${projectName}/src/network/routes ${
  ...
  const processes = [
    ...
    writeFile(
      ...
      t('network/routes/docs.ts.ejs', { email, projectName })
    ),
    writeFile(
...
const main = async ({
  ...
  await network({ projectName, email, graphQL, dbIsSQL })
