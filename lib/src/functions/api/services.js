...
${projectName}/src/services/utils/messages`
  ...
  const t = templatePath => renderTemplate(`api/services/${templatePath}`)

  await Promise.all([
    writeFile(`${projectName}/src/services/index.ts`, t('index.ts.ejs')),
    writeFile(`${projectName}/src/services/BaseHttp.ts`, t('BaseHttp.ts.ejs')),
    writeFile(`${projectName}/src/services/User.ts`, t('User.ts.ejs')),
    writeFile(
    ...
    writeFile(
      ...
      t('utils/messages/index.ts.ejs')
    ),
    writeFile(
      `${projectName}/src/services/utils/messages/user.ts`,
      t('utils/messages/user.ts.ejs')
    )
  ])
}
